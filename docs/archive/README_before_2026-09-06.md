...
> **Revision notice (MEAS-D-26-10542):** The original headline results below
> were produced by a protocol that used the current complete window to choose
> rates for that same window. They are retained only as legacy provenance and
> must not be cited or submitted. The corrected source of truth is
> `REVISION_STATUS.md`, `paper/main_revised.tex`, and the `causal_*` result files.
